            ...
            <InnerContainer>
                ...
                <Formik
                    ...
                    onSubmit={(values) => {
                        ...
                        // 2. CHECK FOR TEST ACCOUNT SUCCESS
                        if (values.email === TEST_USER && values.password === TEST_PASS) {
                            console.log("Test Login Successful! Entering Tabs...");
                            
                            /* MAGIC FIX: Routing to '/(tabs)' instead of a filename 
                               This forces Expo Router to load your app/(tabs)/_layout.tsx
                               which contains your Navbar code.
                            */
                            router.replace('/(tabs)/UserHome'); 
                            return;
                        }

                        // 3. CHECK FOR TEST ACCOUNT WRONG PASSWORD
                        if (values.email === TEST_USER && values.password !== TEST_PASS) {
                            Alert.alert("Login Failed", "Incorrect password for this test account.");
                            return;
                        }

                        // 4. YOUR EXISTING REGEX (For everyone else)
                        ...
                        // 5. DEFAULT ERROR
                        Alert.alert("Account Not Found", "Sign up now!");
                    }}
                >
